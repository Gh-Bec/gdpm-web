...
var ANNOCUNCEMENT_URL = '/tAnnouncement';

export async function requestUpdate(params) {
    return service({
        url: GDPM_TEACHER + ANNOCUNCEMENT_URL + '/update',
        method: 'POST',
        data: params,
...
export async function requestInsert(params) {
    return service({
        url: GDPM_TEACHER + ANNOCUNCEMENT_URL + '/add',
        method: 'POST',
        data: params,
        headers: {'Content-type': POST_HEADER_URLENCODED},
        timeout: 20 * 1000
...
export async function requestHandleDelete(params) {
    return service({
        url: GDPM_TEACHER + ANNOCUNCEMENT_URL + '/delete',
        method: 'POST',
        data: params,
        headers: {'Content-type': POST_HEADER_URLENCODED}
    }).then(response => {
...
export async function requestOnSubmit(current, size, keywords, tid) {
    return service({
        url: GDPM_TEACHER + ANNOCUNCEMENT_URL +  '/findPage',
        method: 'GET',
        params: {
            "current" : current,
            "size" : size,
