...
import {requestFileUpload, requestInsertMid, requestMidFindPage, requestIsMidPass} from '../request/student/SubmitApi';
import {isUserInfoNotNull, isUserDtoNotNull} from '../../js/global/PublicMethods'

export function isMidPassing(that) {
    ...
    requestIsMidPass(sid).then(response => {
...
export function midFindPageing(that) {
    var sid = JSON.parse(window.localStorage.getItem('userDTO')).id;
    ...
    requestMidFindPage(that.current, that.size, sid).then(response => {
...
export function insertMiding(that) {
    var sid = JSON.parse(window.localStorage.getItem('userDTO')).id;
    if(sid == null || sid.length == 0) {
        Message({
            message: '请求参数错误',
    ...
    return requestInsertMid(params);
}
...
export function currentChangeing(that, cpage) {
    ...
    requestMidFindPage(cpage, that.size, sid).then(response => {
        if(response.result == null) {
            return;
        }
        that.total = response.total;
