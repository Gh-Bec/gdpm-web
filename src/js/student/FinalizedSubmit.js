...
import {requestFileUpload, requestInsertDefinte, requestDefineFindPage, requestIsFinalizedPass} from '../request/student/SubmitApi';
import {isUserInfoNotNull, isUserDtoNotNull} from '../../js/global/PublicMethods'

export function isFinalizedPassing(that) {
    ...
    requestIsFinalizedPass(sid).then(response => {
...
export function defineFindPageing(that) {
    var sid = JSON.parse(window.localStorage.getItem('userDTO')).id;
    ...
    requestDefineFindPage(that.current, that.size, sid).then(response => {
...
export function insertDefinteing(that) {
    var sid = JSON.parse(window.localStorage.getItem('userDTO')).id;
    if(sid == null || sid.length == 0) {
        Message({
            message: '请求参数错误',
    ...
    return requestInsertDefinte(params);
}
...
export function currentChangeing(that, cpage) {
    ...
    requestDefineFindPage(cpage, that.size, sid).then(response => {
        if(response.result == null) {
            return;
        }
        that.total = response.total;
