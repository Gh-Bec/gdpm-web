...
import {requestFileUpload, requestComFindPageing, requestInsertCom, requestIsComPass} from '../request/student/SubmitApi';
import {isUserInfoNotNull, isUserDtoNotNull} from '../../js/global/PublicMethods'

export function isComPassing(that) {
    ...
    requestIsComPass(sid).then(response => {
...
export function insertComing(that) {
    var sid = JSON.parse(window.localStorage.getItem('userDTO')).id;
    ...
    return requestInsertCom(params);
}

export function comFindPageing(that) {
    var sid = JSON.parse(window.localStorage.getItem('userDTO')).id;
    if(sid == null || sid.length == 0) {
        Message({
            message: '请求参数错误',
    ...
    requestComFindPageing(that.current, that.size, sid).then(response => {
...
export function currentChangeing(that, cpage) {
    ...
    requestComFindPageing(cpage, that.size, sid).then(response => {
        if(response.result == null) {
            return;
        }
        that.total = response.total;
