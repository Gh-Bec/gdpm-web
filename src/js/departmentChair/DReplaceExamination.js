...
import {requestReplaceFindPage, requestReplaceAgree, requestReplaceRefused} from '../request/departmentChair/DSubjectExaminationApi';

export function replaceFindPageing(that) {
    ...
    requestReplaceFindPage(that.current, that.size, tid, keywords).then(response => {
...
export function currentChangeing(that, cpage) {
    ...
    requestReplaceFindPage(cpage, that.size, tid, keywords).then(response => {
        if(response.result == null) {
            return;
        }
        that.total = response.total;
...
export function agreeing(that, index, row) {
    ...
    requestReplaceAgree(params).then(response => {
...
export function refuseding(that) {
    ...
    requestReplaceRefused(params).then(response => {
        if(response.data.state >= 1) {
            Message.success('审批成功');
            that.replaceFindPage();
        } else {
