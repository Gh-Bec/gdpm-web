...
import {requestDeclarationFindPage, requestDeclaretionAgree, requestDeclaretionRefused} from '../request/departmentChair/DSubjectExaminationApi';

export function declarationFindPageing(that) {
    ...
    requestDeclarationFindPage(that.current, that.size, tid, keywords).then(response => {
...
export function currentChangeing(that, cpage) {
    ...
    requestDeclarationFindPage(cpage, that.size, tid, keywords).then(response => {
        if(response.result == null) {
            return;
        }
        that.total = response.total;
...
export function agreeing(that, index, row) {
    ...
    requestDeclaretionAgree(params).then(response => {
...
export function refuseding(that) {
    ...
    requestDeclaretionRefused(params).then(response => {
        if(response.data.state >= 1) {
            Message.success('审批成功');
            that.declarationFindPage();
        } else {
 ...
 * @param {*} that 
 */
export function toIntroduceViewing(that, index, row) {
    that.introduce = row.introduce;
}
