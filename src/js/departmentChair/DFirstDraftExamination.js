...
import {requestFirstFindPage, requestFirstAgree, requestFirstRefused} 
...
export function firstFindPageing(that) {
    ...
    requestFirstFindPage(that.current, that.size, tid, keywords).then(response => {
...
export function currentChangeing(that, cpage) {
    ...
    requestFirstFindPage(cpage, that.size, tid, keywords).then(response => {
        if(response.result == null) {
            return;
        }
        that.total = response.total;
...
export function agreeing(that, index, row) {
    ...
    requestFirstAgree(params).then(response => {
...
export function refuseding(that) {
    ...
    requestFirstRefused(params).then(response => {
        if(response.data.state >= 1) {
            Message.success('审批成功');
            that.firstFindPage();
        } else {
