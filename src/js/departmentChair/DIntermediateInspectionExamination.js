...
import {requestMidFindPage, requestIntermediateAgree, requestIntermediateRefused} 
...
export function midFindPageing(that) {
    ...
    requestMidFindPage(that.current, that.size, tid, keywords).then(response => {
...
export function currentChangeing(that, cpage) {
    ...
    requestMidFindPage(cpage, that.size, tid, keywords).then(response => {
        if(response.result == null) {
            return;
        }
        that.total = response.total;
...
export function agreeing(that, index, row) {
    ...
    requestIntermediateAgree(params).then(response => {
...
export function refuseding(that) {
    ...
    requestIntermediateRefused(params).then(response => {
        if(response.data.state >= 1) {
            Message.success('审批成功');
            that.midFindPage();
        } else {
