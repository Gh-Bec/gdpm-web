...
import {requestCommitFindPage} from '../request/departmentChair/DProcessExaminationApi';
...
export function commitFindPageing(that) {
    ...
    requestCommitFindPage(that.current, that.size, tid, keywords).then(response => {
...
export function currentChangeing(that, cpage) {
    ...
    requestCommitFindPage(cpage, that.size, tid, keywords).then(response => {
        if(response.result == null) {
            return;
        }
        that.total = response.total;
