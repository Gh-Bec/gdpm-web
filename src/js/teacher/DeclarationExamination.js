...
import {requestDeclarationFindPage, requestDeclaretionAgree, requestDeclaretionRefused} from '../request/teacher/ExaminationApi';
...
export function declarationFindPageing(that) {
    ...
    requestDeclarationFindPage(that.current, that.size, tid).then(response => {
...
export function currentChangeing(that, cpage) {
    ...
    var tid = JSON.parse(window.localStorage.getItem('userDTO')).id;
    isUserInfoNotNull(tid, that);
    requestDeclarationFindPage(cpage, that.size, tid).then(response => {
        if(response.result == null) {
            return;
        }
        that.total = response.total;
