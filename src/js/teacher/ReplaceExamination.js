...
import {requestReplaceFindPage, requestReplaceAgree, requestReplaceRefused} from '../request/teacher/ExaminationApi';
...
export function replaceFindPageing(that) {
    ...
    requestReplaceFindPage(that.current, that.size, tid).then(response => {
...
export function currentChangeing(that, cpage) {
    ...
    var tid = JSON.parse(window.localStorage.getItem('userDTO')).id;
    isUserInfoNotNull(tid, that);
    requestReplaceFindPage(cpage, that.size, tid).then(response => {
        if(response.result == null) {
            return;
        }
        that.total = response.total;
