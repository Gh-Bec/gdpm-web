...
import {requestOpenFindPage, requestOpenRecordAgree, requestOpenRecordRefused} 
...
export function openFindPageing(that) {
    ...
    requestOpenFindPage(that.current, that.size, tid).then(response => {
...
export function currentChangeing(that, cpage) {
    ...
    var tid = JSON.parse(window.localStorage.getItem('userDTO')).id;
    isUserInfoNotNull(tid, that);
    requestOpenFindPage(cpage, that.size, tid).then(response => {
        if(response.result == null) {
            return;
        }
        that.total = response.total;
...
export function agreeing(that, index, row) {
    ...
    requestOpenRecordAgree(params).then(response => {
...
export function refuseding(that) {
    ...
    requestOpenRecordRefused(params).then(response => {
        if(response.data.state >= 1) {
            Message.success('审批成功');
            that.openFindPage();
        } else {
