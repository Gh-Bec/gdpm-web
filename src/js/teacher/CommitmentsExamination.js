...
import {requestCommitFindPage, requestCommitmentAgree, requestCommitmentRefused} 
        from '../request/teacher/ExaminationApi';

export function commitFindPageing(that) {
    ...
    requestCommitFindPage(that.current, that.size, tid).then(response => {
...
export function currentChangeing(that, cpage) {
    ...
    var tid = JSON.parse(window.localStorage.getItem('userDTO')).id;
    isUserInfoNotNull(tid, that);
    requestCommitFindPage(cpage, that.size, tid).then(response => {
        if(response.result == null) {
            return;
        }
        that.total = response.total;
...
export function agreeing(that, index, row) {
    ...
    requestCommitmentAgree(params).then(response => {
...
export function refuseding(that) {
    ...
    requestCommitmentRefused(params).then(response => {
        if(response.data.state >= 1) {
            Message.success('审批成功');
            that.commitFindPage();
        } else {
