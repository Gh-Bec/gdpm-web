<template>
    <div id="smain8" class="bodys smidf">
...
<script>
    ...
    import {currentChangeing, fileUploading, insertDefinteing, defineFindPageing, isFinalizedPassing} from '../../js/student/FinalizedSubmit';
    ...
    export default {
        ...
        methods: {
            refresh() {
                var that = this;
                that.isPass = true;
                that.isFinalizedPass();
                that.defineFindPage();
            },
            async isFinalizedPass() {
                var that = this;
                isFinalizedPassing(that);
            },
            async defineFindPage() {
                var that = this;
                defineFindPageing(that);
            },
            async insertDefinte() {
                var that = this;
                insertDefinteing(that).then(response => {
                    if(response.msg == 'success' && response.msg == 'success') {
                        if(response.data.state >= 1) {
                            that.defineFindPage(that);
                        }
                    } else {
                        MessageBox({
                            title: '提示',
