<template>
    <div id="smain7" class="bodys smidf">
        ...
        <div class="stable">
            ...
            <div class="upload">
                <el-upload class="upload-demo" drag multiple accept=".doc,.docx,.pdf" :disabled="isPass"
...
<script>
    ...
    import {currentChangeing, fileUploading, insertFirsting, firstFindPageing, isFirstPassing} from '../../js/student/FirstDraftSubmit'
    ...
    export default {
        ...
        methods: {
            ...
            async firstFindPage() {
                var that = this;
                firstFindPageing(that);
            },
            async insertFirst() {
                var that = this;
                insertFirsting(that).then(response => {
                    if(response.msg == 'success' && response.msg == 'success') {
                        if(response.data.state >= 1) {
                            that.firstFindPage(that);
                        }
                    } else {
                        MessageBox({
                            title: '提示',
