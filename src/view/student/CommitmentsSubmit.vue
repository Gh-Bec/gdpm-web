<template>
    <div id="smain4" class="bodys scomopen">
...
<script>
    ...
    import {currentChangeing, fileUploading, comFindPageing, insertComing, isComPassing} from '../../js/student/CommitmentsSubmit'
    ...
    export default {
        ...
        methods: {
            ...
            async insertCom() {
                var that = this;
                insertComing(that).then(response => {
                    if(response.msg == 'success' && response.msg == 'success') {
                        if(response.data.state >= 1) {
                            that.comFindPage(that);
                        }
                    } else {
                        MessageBox({
                            title: '提示',
