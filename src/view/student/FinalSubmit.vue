<template>
    <div id="smain9" class="bodys smidf">
...
<script>
    ...
    import {uploadSuccessing, uploadErroring, getState, previewing, downloading} from '../../js/global/PublicMethods'
    import {currentChangeing, fileUploading, insertFinaling, finalFindPageing, isFinalPassing} from '../../js/student/FinalSubmit';
    ...
    export default {
        ...
        methods: {
            ...
            async finalFindPage() {
                var that = this;
                finalFindPageing(that);
            },
            async insertFinal() {
                var that = this;
                insertFinaling(that).then(response => {
                    if(response.msg == 'success' && response.msg == 'success') {
                        if(response.data.state >= 1) {
                            that.finalFindPage(that);
                        }
                    } else {
                        MessageBox({
                            title: '提示',
